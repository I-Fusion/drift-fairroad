...
- **Server** (`fl_server_classification.py`): Coordinates rounds, aggregates client weights (FedAvg, FedAvgM, or weighted), saves checkpoints.
- **Clients** (`fl_client_classification.py`): Load per-client data, train on windows, submit updates. Support for packet-only or GPS/IMU data.
- **Run script** (`run_fl_system_classification.py`): Starts server and clients from the project root; uses a single config module (e.g. `config_classification` or `config_regression`).
...
| File | Description |
|------|-------------|
| `run_fl_system_classification.py` | Main entry: starts FL server and client processes. |
| `fl_server_classification.py` | FL server: registration, model distribution, aggregation, checkpoints. |
| `fl_client_classification.py` | FL client: data load, training, submit updates. |
...
| `data_preprocessing_classification.py` | Builds sliding-window data from packet or GPS/IMU CSVs. |
...
**From the project root** (not from inside `fl-classification`):
...
3. **Run FL regression** (if you have GPS/IMU data):
   ```bash
   python fl-classification/run_fl_system_classification.py --data-dir data/train/gps-imu --config config_regression --learning-mode regression
   ```

Checkpoints are written to `checkpoints/` (server) and `checkpoints/clients/` (client checkpoints). Use `evaluate_fl_system.py` in the project root to evaluate; see `EVALUATE_FL_SYSTEM.md`.

---

## Key Options (`run_fl_system_classification.py`)
...
- **Evaluation**: From project root, `python evaluate_fl_system.py --checkpoint-dir checkpoints --data-dir data/train/packets --task classification --config config_classification`. Covers detection metrics (AUROC, precision, recall, F1, false alarm rate), drift, mission continuity, and efficiency — see `EVALUATE_FL_SYSTEM.md`.
