...
```bash
python evaluate_fl_system.py --checkpoint-dir checkpoints --data-dir data/validate/packets --task classification --config config_classification
```
...
- **`--checkpoint-dir`** (required)  
  Directory containing server checkpoints. Files should be named `server_round_XX.pt` (from `fl-classification/run_fl_system_classification.py`).
...
| `--output-dir` | Where to save reports and plots (default: `evaluation_results`). |
...
This loads all `server_round_*.pt` checkpoints, builds sliding windows from `packet_client_*.csv` using `config_classification` (WINDOW_SIZE, OVERLAP, PACKET_FEATURES, LABEL_COL), and writes metrics and plots to `evaluation_results/`.

### 6.2. GPS/IMU regression (`config_regression`)

```bash
python evaluate_fl_system.py --checkpoint-dir checkpoints --data-dir data/validate/gps-imu --task regression --config config_regression
```
...
```bash
python evaluate_fl_system.py --checkpoint-dir checkpoints --data-dir data/train/packets --task classification --config config_classification --max-false-alarms-per-hour 5 --window-duration-sec 1.0 --attack-onset-file path/to/attack_onsets.json
```
...
With `--output-dir` (default: `evaluation_results/`):
...
- **Import path**: The script adds `fl-classification/` to `sys.path` so it can import `DataPreprocessorClassification`; run from the **project root**.
...
**See also**: `fl-classification/README.md` (training), `evaluate_fl_system.py` docstrings and comments.
